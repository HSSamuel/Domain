...
export default function LandingPage() {
  ...
  // FIXED: Added loading state for the join session form
  ...
  const handleJoin = (e: React.FormEvent) => {
    ...
    if (pin.trim().length === 6) {
      setIsJoining(true); // Trigger the loading spinner
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 relative overflow-hidden selection:bg-indigo-100">
      ...
      <header className="px-4 md:px-8 py-4 flex justify-between items-center max-w-7xl mx-auto w-full relative z-20">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <button 
            ...
            className="inline-flex items-center justify-center gap-2 text-sm font-black text-white hover:text-white bg-slate-900 hover:bg-indigo-600 transition-all px-5 py-2.5 rounded-xl shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95 disabled:bg-indigo-400 disabled:opacity-80 disabled:pointer-events-none"
          >
            {isPortalLoading ? (
              <>
      ...
      <div className={`md:hidden fixed w-full bg-white/95 backdrop-blur-xl shadow-2xl border-b border-slate-200 z-50 transition-all duration-300 ease-out ${isMenuOpen ? 'top-[72px] opacity-100 visible' : 'top-10 opacity-0 invisible pointer-events-none'}`}>
        <div className="flex flex-col px-6 py-6 gap-2">
          ...
          <button 
            onClick={() => { setIsMenuOpen(false); handlePortalNavigation(); }} 
            disabled={isPortalLoading}
            className="mt-4 flex items-center justify-center gap-2 w-full text-center text-sm font-black text-white bg-slate-900 hover:bg-indigo-600 py-4 rounded-xl transition-colors shadow-md disabled:bg-indigo-400 disabled:pointer-events-none"
          >
            {isPortalLoading ? (
              <>
                <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 md:px-6 text-center max-w-4xl mx-auto relative z-0 py-20">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[2rem] shadow-lg border border-slate-100 w-full max-w-sm mx-auto relative overflow-hidden group hover:shadow-xl transition-all duration-500">
          ...
          <form onSubmit={handleJoin} className="flex flex-col gap-3">
            ...
            />
            {/* FIXED: Added disabled state, flex layout, and spinner to Enter Session button */}
            <button 
              type="submit" 
              disabled={isJoining}
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 disabled:pointer-events-none text-white font-black text-base py-3.5 rounded-xl transition-all shadow-md active:scale-95 mt-1"
            >
              {isJoining ? (
                <>
