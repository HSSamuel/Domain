...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (hasAccess === 'granted') {
      ...
      // Fetch the facilitator's name for the profile badge
  ...
  const handleLogout = () => {
    // Securely clear all tokens and state
  ...
  // Dynamically set the header title based on the current route
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans selection:bg-indigo-100 text-slate-900">
          ...
          <div className="w-64 h-full bg-slate-900 shadow-2xl p-4 flex flex-col border-r border-slate-700" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-8 px-2 mt-2">
              <Image 
                ...
                width={40} 
                height={40} 
                className="w-10 h-10 rounded shadow-md flex-shrink-0 object-contain bg-white p-[2px]" 
              />
              <span className="font-black tracking-tight text-lg text-white">DomainAssess</span>
            </div>
            
            <nav className="flex-1 flex flex-col space-y-2">
              <Link href="/admin/dashboard" onClick={() => setIsMobileMenuOpen(false)} className={`p-3 rounded-xl font-bold flex items-center gap-3 ${pathname === '/admin/dashboard' ? 'bg-indigo-600/20 text-indigo-400' : 'text-slate-300 hover:bg-slate-800'}`}>
                Dashboard
              </Link>
              <Link href="/admin/create-bank" onClick={() => setIsMobileMenuOpen(false)} className={`p-3 rounded-xl font-bold flex items-center gap-3 ${pathname === '/admin/create-bank' ? 'bg-indigo-600/20 text-indigo-400' : 'text-slate-300 hover:bg-slate-800'}`}>
                Create Assessment
              </Link>
              <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="p-3 rounded-xl font-bold flex items-center gap-3 text-slate-300 hover:bg-slate-800">
                Live Arena (Home)
              </Link>
            </nav>
            ...
            <div className="pt-4 mt-auto border-t border-slate-800 space-y-4">
              ...
              <button onClick={handleLogout} className="w-full p-3 rounded-xl font-bold flex items-center gap-3 text-rose-400 hover:bg-rose-500/10">
                Log Out
              </button>
            </div>
          </div>
      ...
      <aside className={`hidden md:flex bg-slate-900 text-slate-300 flex-col transition-all duration-300 ease-in-out relative shadow-xl z-40 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
        ...
        <div className="h-14 flex items-center justify-center border-b border-slate-800 shrink-0 px-3">
          ...
          {/* UPDATED: Dynamic sizing based on isSidebarOpen */}
          <Image 
            src="/logo.png" 
            alt="DomainAssess Logo" 
            width={36} 
            height={36} 
            className={`rounded shadow-md flex-shrink-0 object-contain bg-white transition-all duration-300 ${isSidebarOpen ? 'w-9 h-9 p-[2px]' : 'w-7 h-7 p-1'}`} 
          />
          
          {/* We wrap the text in a span with transition classes to prevent layout jumping when collapsing */}
          <span className={`ml-3 font-black text-white text-base tracking-tight whitespace-nowrap transition-all duration-300 ${isSidebarOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0 overflow-hidden ml-0'}`}>
            DomainAssess
          </span>
        </div>
        ...
        <div className="p-3 border-t border-slate-800 flex flex-col gap-2">
          ...
          {/* Logout Button */}
          <button onClick={handleLogout} className={`w-full flex items-center p-2.5 rounded-lg transition-colors text-rose-400 hover:bg-rose-500/10 hover:text-rose-300 ${isSidebarOpen ? '' : 'justify-center'}`} title="Log Out">
            <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
            {isSidebarOpen && <span className="ml-3 font-bold text-sm whitespace-nowrap">Log Out</span>}
          </button>
